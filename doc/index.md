...
You can think about it as a climb up to get all non-reset values necessary for the calculation (i.e. Cells not impacted by the `set_value()`), and a climb down to calculate at each node its new value, until you reach the output Cell.
...
This is because Excel chooses to operate only the Cells associated to the calling Cell, meaning the Cells that are in-line horizontally or vertically with the Cell holding the formula. If one of the Ranges is not associated to the calling Cell, it outputs a `#VALUE!` error.
