...
from koala.ast.Range import Range, check_array
...
class Test_Excel(unittest.TestCase):
    ...
    def test_check_array(self):
    	self.assertEqual(check_array(Range([1, 2, 3, 4]), 2), 3)

    def test_check_array_raises_error(self):
        with self.assertRaises(ValueError):
            check_array(Range([1, 2, 3, 4]), 5)

    # ADD
    def test_add_array(self):
        self.assertEqual(Range.add(Range([1, 2, 3, 4]), Range([4, 3, 2, 1]), 2), 5) # 3 + 2 = 5
